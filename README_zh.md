...
**使用 Gemini 2.5:**
```bash
python -m src.cli generate --prompt "解释量子力学" --model gemini-2.5-flash
```
**使用 Qwen Max:**
```bash
...
├── src/
│   ├── budget/         # 预算控制 & 定价逻辑
│   ├── config/         # 配置加载器 & Pydantic 模型
│   ├── doctor/         # 连通性 & 健康检查
│   ├── providers/      # 适配器 (Gemini, Qwen, Echo)
│   ├── utils/          # 网络工具
│   ├── client.py       # 主入口
│   └── cli.py          # 命令行接口
├── tests/              # Pytest 测试套件
...
## 🤝 贡献
1.  Fork 本仓库。
2.  在 `src/providers/` 中添加新的 Provider。
3.  在 `src/client.py` 中注册它。
4.  提交 PR!
