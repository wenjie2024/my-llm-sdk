...
### 0.4.0-C：TTS Streaming Refactor (Advanced Prosody)
**范围**
* `QwenProvider` 支持 Long-lived WebSocket 连接
* 流式输入文本（chunked text）而不中断 Session
* 解决短句拼接导致的语调/语速不一致问题

**DoD**
* 长文本 TTS 生成时，语调连贯，无明显断裂感
* 单次 Session 可处理 >10k 字符的流式输入

### 0.4.1：Gemini Image Generation Enhancement ✅
**范围**
* `image_config` 参数支持 (`image_size`, `aspect_ratio`)
* 灵活输入支持 (`str`, `PIL.Image`, `ContentPart` 混合)
* 安全拦截检测 (`safety_blocked` 状态)
...
### V0.5.1 模型定价同步（Pricing Registry）
* 从 `llm.project.yaml` 读取 `input_token/output_token` 价格
* 为 Ledger 提供真实的 `cost_usd` 结算能力

### V0.5.2 报警聚合（Alerting）
* 当今日消耗达到阈值（80/100%）时，通过 Log/CLI 实现主动显著报警

### V0.5.3 测试集成（Pytest Integration）
* 将 `verify_*.py` 的验证逻辑正式集成到 `pytest` 自动化流水线
